-- Test sheet's core AS/NZS 3000 column set needs two test_types that don't
-- exist yet: "Correct Circuit Connections" (pass/fail only, no measured
-- value) and "RCD Trip Current" (mA) as a separate column from the
-- existing "RCD/RCBO Testing" row, which the test sheet treats as RCD trip
-- time (ms). Not renaming "RCD/RCBO Testing" itself -- that would silently
-- change the displayed label on every existing historical test record
-- referencing it, which is exactly the kind of change this table must
-- never make.
--
-- Backfills every existing business (test_types is business-scoped, so a
-- new row has to be inserted per business, not seeded from a shared
-- default), then updates seed_new_business() so future signups get both
-- from day one.
insert into public.test_types (business_id, name, default_unit, is_custom)
select b.id, seed.name, seed.default_unit, false
from public.businesses b
cross join (values
  ('Correct Circuit Connections', null),
  ('RCD Trip Current', 'mA')
...
begin
  ...
  insert into public.test_types (business_id, name, default_unit, is_custom) values
    ...
    (p_business_id, 'Correct Circuit Connections', null, false),
    (p_business_id, 'RCD/RCBO Testing', 'ms', false),
    (p_business_id, 'RCD Trip Current', 'mA', false),
